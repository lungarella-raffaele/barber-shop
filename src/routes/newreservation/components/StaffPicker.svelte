...
<svelte:boundary>
	...
	{:then data}
		<RadioGroup.Root bind:value class="flex flex-wrap gap-2" disabled={data?.length === 1}>
			{#each data ?? [] as member, index (index)}
				<RadioGroup.Item value={member.id}>
					{#snippet children({ checked })}
						<div
							class="flex items-center rounded-sm border bg-background p-3 transition-all duration-200 ease-in-out hover:border-primary {checked
